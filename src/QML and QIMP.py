...
def calculate_gp_image(green_channel, red_channel, G=0.5):
    ...
    gp_array = (green_channel - G * red_channel) / (green_channel + G * red_channel + 0)
    gp_array = np.clip(gp_array, -1, 1)

    # Interpolazione per 16-bit
    gp_image_16bit = np.interp(gp_array, [-1, 1], [0, 65535]).astype(np.uint16)
    ...
    for image_file in image_files:
        try:
            ...
            if image_array.ndim == 3 and image_array.shape[2] >= 3:
                ...
                # Applica i filtri (senza soglia) e calcola l'immagine GP
                filtered_red_channel = apply_filters(red_channel)
                filtered_green_channel = apply_filters(green_channel)

                gp_image = calculate_gp_image(filtered_green_channel, filtered_red_channel)  # Inverto red e green

                # Visualizza prima il canale verde, poi il rosso e infine l'immagine GP
                visualize_channels_and_gp(filtered_green_channel, filtered_red_channel, gp_image)
