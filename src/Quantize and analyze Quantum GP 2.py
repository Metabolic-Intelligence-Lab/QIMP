...
# Alternative directories if the above doesn't exist
...
def main(image_directory, alpha=1, beta=1, gamma=1):
    ...
    if not os.path.exists(image_directory):
        print(f"⚠️ Directory {image_directory} non trovata, provo fallback...")
        image_directory = fallback_input
        if not os.path.exists(image_directory):
            raise FileNotFoundError(f"Nessuna directory immagini trovata. Controllare: {image_directory}")
    ...
    print(f"📁 Directory input: {image_directory}")
    print(f"📁 Directory output: {output_dir}")
    
    # Get files with better filtering
    try:
        all_files = os.listdir(image_directory)
        input_files = [os.path.join(image_directory, file) for file in all_files if file.endswith('.tif') and 'ch' in file.lower()]
        
        if os.path.exists(output_dir):
            output_files = [os.path.join(output_dir, file) for file in os.listdir(output_dir) if file.endswith('.tif') and 'GP' in file]
        else:
            output_files = []
        ...
        raise
    # Handle case where output files don't exist yet
    if not output_files:
        print("🔶 Nessun file GP trovato, procedo con solo file di input")
        # Create dummy pairs with input files
        pairs = [(input_files[i], input_files[i], None) for i in range(min(len(input_files), 2))]
    else:
        pairs = pair_input_output_files(input_files, output_files)
    
    if not pairs:
        raise ValueError("Nessun file di input valido trovato!")
    
    angles_list, n, m, normalization_factor = load_and_encode_image_pairs(pairs)
    
    # Handle GP image loading
    if pairs[0][2] is not None and os.path.exists(pairs[0][2]):
        gp_image_classic = imageio.imread(pairs[0][2])
    else:
        # Generate GP image from input images if no GP file exists
        print("🧮 Generando immagine GP da file di input...")
        if len(pairs) >= 2:
            img1 = imageio.imread(pairs[0][0])
            img2 = imageio.imread(pairs[1][0]) 
            # Simple GP calculation (you may need to adjust based on your data)
            gp_image_classic = (img1.astype(float) - img2.astype(float)) / (img1.astype(float) + img2.astype(float) + 1e-10)
            gp_image_classic = np.clip(gp_image_classic, -1, 1)
        else:
            # Use first image as reference
            gp_image_classic = imageio.imread(pairs[0][0]).astype(float) / 255.0
    ...
    save_images(classic_images, quantum_images, output_dir)
